...
void fp_montgomery_calc_normalization(fp_int *a, fp_int *b)
{
  ...
  /* CABI: we must find R s.t. R = 2^(k*w) where w is the word size */
  /* CABI: after finding R the (x * R mod m) would be easily div/shift by radix */

  /* compute A = B^(n-1) * 2^(bits-1) */
  if (b->used > 1) {
    /* CABI: find R such that R >= N
     * https://crypto.stackexchange.com/questions/5342/montgomery-exponentiation-selecting-input-value-r-for-a-given-biginteger */ 
...
/* Start: src/numtheory/fp_isprime_ex.c */
/* TomsFastMath, a fast ISO C bignum library.
 *
 * This project is meant to fill in where LibTomMath
 * falls short.  That is speed ;-)
...
/* End: src/numtheory/fp_isprime_ex.c */
