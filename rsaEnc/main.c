...
void writeFile( const unsigned char * buffer, size_t size, char *path)
{
	...
	fp = fopen(path, "ab+");/*se deschide fisierul pentru scriere in binar*/
	 if (fp == NULL)/*se verifica daca fisierul este gol si se genereaza un mesaj de eroare apoi se inchide si se iese din fisier*/
	  {
	...
	fclose(fp);
	/*
	{0xFF, 0xEE} sizeof(.) -> 2, no of el = 2
	{0xEEFF, 0xDDCC} sizeof(.) -> 4, no of el = 2
	*/
}
 ...
 */
void readFile(unsigned char * buffer, char *path);
void readFile(unsigned char * buffer, char *path)/*functia de citire*/
{
  FILE * fp;
  size_t size;
  ...
  else if (fread(buffer, sizeof *buffer, size, fp) != size)/*se verifica daca s a citit intreg fisierul*/
  {
    printf("Error: There was an Error reading the file %s\n", path);
    fclose(fp);
    exit(1);
...
int main(int argc, char *argv[])
{
  ...
  unsigned int inputFileSize, outputFileSize,ModFileSize,ExpFileSize;
  ...
  else
  {
    ...
    printf("inFileSize: %d outFileSize: %d\n ModFileSize: %d ExpFileSize: %d",inputFileSize, outputFileSize, ModFileSize,ExpFileSize);
  }
  ...
  readFile(AliceMod,ModFileName);/**se citeste modulul lui Alice*/

  fp_read_unsigned_bin(&modBn, AliceMod, sizeof(AliceMod));



 if(operation==1)/*encript*/
 {
    /* modulo = 5 bits
     * -------------------------------------------------: message.txt
     * ----- ----- ----- ----- ----- ----- ----- -----
     */
	 readFile(AlicePubExp, ExpFileName);/*se citeste exponentul public a lui Alice*/
	 fp_read_unsigned_bin(&expPubBn, AlicePubExp, sizeof(AlicePubExp));/*converteste din bytearray in bignumber*/

	 readFile( message, inputFileName);
	 bytesRead= strlen((const char *)message);

	 for(unsigned int i=0; i<=strlen((const char *)message)/MOD_LEN ; i+=MOD_LEN)
	 {
		 ...
		 fp_read_unsigned_bin(&msgBn, &message[i],toProcess);/*transforma mesajul din BigNumber intr un plain message*/

		 fp_exptmod(&msgBn, &expPubBn, &modBn, &cyphertextBn);/*functia de criptare x^e mod n*/

	     fp_to_unsigned_bin(&cyphertextBn, cyphertext);

	     writeFile(cyphertext, sizeof(cyphertext), outputFileName);/*functie care scrie mesajul criptat*/
	 }

 }
 else /*decript*/
 {
	 readFile(AlicePrivExp, ExpFileName);/*se citeste exponentul privat a lui Alice*/
	 fp_read_unsigned_bin(&expPrivBn, AlicePrivExp, sizeof(AlicePrivExp));/*converteste din bytearray in bignumber*/

	 readFile( cyphertext, inputFileName);
	 bytesRead=strlen((const char *)cyphertext);
	 for(unsigned int i=0; i<=strlen((const char *)cyphertext)/MOD_LEN ; i+=MOD_LEN)
		 {
		        unsigned int toProcess;
				 bytesRead-=MOD_LEN;
				 if(bytesRead<0)
	  ...
	  fp_read_unsigned_bin(&cypBn,  &cyphertext[i],toProcess);
	  ...
	  writeFile(message, sizeof(message), outputFileName);/*se genereaza mesajul care a fost decriptat*/
 }
 }
